...
// Temporary: merge duplicate Drive folders
app.post('/api/merge-duplicate-folders', async (req, res) => {
  try {
    const folderId = req.body.folderId || '1-CMdxzcb2CmEOr5hFokMELwkVsw5gc1Z';
    const drive = getDriveClient();
    const logs: string[] = [];

    // List all folders
    const folders: { id: string; name: string; createdTime: string }[] = [];
    ...
    logs.push(`Found ${folders.length} folders total`);

    // Group by name
    const byName: Record<string, typeof folders> = {};
    ...
    logs.push(`Found ${duplicates.length} duplicate names`);
    ...
    for (const [name, arr] of duplicates) {
      ...
      logs.push(`"${name}" — ${arr.length} copies, keeping ${main.id}`);

      for (const dupe of dupes) {
        // List files in duplicate
        const filesRes: any = await drive.files.list({
          q: `'${dupe.id}' in parents and trashed=false`,
        ...
        if (files.length > 0) {
          logs.push(`  Moving ${files.length} files from ${dupe.id}`);
        ...
        logs.push(`  Deleted duplicate ${dupe.id}`);
        mergedCount++;
      }
    }

    logs.push(`Done! Merged ${mergedCount} duplicate folders.`);
    res.json({ success: true, mergedCount, duplicateNames: duplicates.length, logs });
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
});
